...
int phpc_gemm_cublas(const double *a, int lda, const double *b, int ldb, double *c, int ldc, int m, int k, int n, int gpu_count, int grid_width, int grid_height, int block_width) {
  ...
  if (cublasCreate(&handle) != CUBLAS_STATUS_SUCCESS) {
    ...
    for (int gpu = 0; gpu < gpu_count; gpu++) {
      ...
      cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, dev_n, m, k, &alpha, dev_buffers_b[gpu], n, dev_buffers_a[gpu], k, &beta, dev_buffers_c[gpu], n);
...
int phpc_gemm_summa_cublas(MPI_Comm grid_comm, const double *A, const double *B, double *C, int lda, int ldb, int ldc, int matrices_width, int gpu_count, int grid_width, int grid_height, int block_width) {
  if (grid_comm == NULL || A == NULL || B == NULL || C == NULL || lda <= 0 || ldb <= 0 || ldc <= 0)
    return 1;
  ...
  for (int k = 0; k < lcm; k++) {
    ...
    /* compute product of the blocks */
    phpc_gemm_cublas(block_a, block_lda, block_b, block_ldb, offset_c, ldc, local_A_rows, panel_K_dim, local_B_cols, gpu_count, grid_width, grid_height, block_width);
  }
