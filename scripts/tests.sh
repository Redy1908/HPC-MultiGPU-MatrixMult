...
# bin/iterative.out 16 > csv/iterative.csv
# bin/iterative.out 32 >> csv/iterative.csv
# process() {
#     touch "$1_temp_lock"
...
#         echo "$size,$proc,$gpu,$blocks,$threads,$tot_threads,$time,$kernel_time,$cublas,$speedup,$efficiency,$speedup_kernel,$efficiency_kernel" >> "$1_temp_lock"
#     done < $1
...
cat csv/test0_*.csv | sort -h > csv/test0.csv && rm csv/test0_*.csv
